...
async def on_receive_video(websocket: WebSocket):
    ...
    while True:
        ...
        score_threshold = [0.03, 0.05, 0.05, 0.05, 0.05, 0.05, 0.05, 0.05, 0.05, 0.007]
        ...
        for i, score in enumerate(scores):
            if additional_info[i] >= 0:
                if score > score_threshold[additional_info[i]]:
                    classes.append(additional_info[i].tolist())
                    metadata.append(bbox[i].tolist())

        # 결과를 클라이언트로 전송
        await websocket.send_json({"class": classes,"bboxes": metadata})
